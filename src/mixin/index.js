...
export default {
    methods: {
        vuexFromStorage(module, key) {
            store.commit(`${module}/${key}`, this.storageLoad(`${module}-${key}`) ?? "");
            // console.log(key)
        ...
        storageSave(module, key, value) {
            const json = JSON.stringify(value);
            const encryptedValue = this.encryptString(json);
            const encryptedKey = this.encryptString(`${module}-${key}`);
